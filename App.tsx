...
const REACTION_GRADES_CONFIG = [
  ...
  { min: 451, max: 600, color: 'border-orange-900' }, // Corrected Bronze color
 ...
 * This is good for modeling wait times, making shorter waits
 * much more common than longer ones.
 * @param rate - The rate parameter (lambda), which is 1 / desired_average.
...
const App = () => {
  ...
  const showTarget = useCallback(() => {
    ...
    const areOverlapping = (p1: { top: number, left: number }, p2: { top: number, left: number }) => {
        ...
        return distance < DOT_SIZE * 2; // Check if they are too close
    };

    // From round 3 onwards, there's a chance for a decoy
  ...
  const scheduleNextTarget = useCallback(() => {
    clearTimer();
    setTargetPosition({ top: -100, left: -100 }); // Hide dots while waiting
